...
public:
	...
	double solveDistance(coordinateList );
private:
...
void convexHull::readCoords (coordinateList &L, int number = 10)
{
	...
	while (number > 0)
	{
		...
		cout << "I now have " << L.size() + 1 << " elements\n";
	...
	if (L.size() <= 3) {
		...
		cout << "\nThat's it. We have them all " << L.size() +1 << " points now.\n\n";
	}
}
...
double convexHull::solveDistance(coordinateList P) {
	double total = 0, add = 0;
	for (unsigned int i = 0; i < P.size() - 1; i++) {
		add = findDistance(getCoord(P,i),getCoord(P,i+1));
		total += add;
	}
	return total;
}

double convexHull::findDistance(coordinate A, coordinate B) {
	double d = 0; // he he -- I'm so immature...
	int x1 = A.x, x2 = B.x, y1 = A.y, y2 = B.y, diffX = 0, diffY = 0;
	
	diffX = x2 - x1;
	diffY = y2 - y1;
	
	d = (diffX ^ 2) + (diffY ^ 2);
	d = fabs(d);	// Make positive using |d| - absolute value
	d = sqrt(d);	// Square root twice to root original d
	return d;
}
...
coordinateList convexHull::dcMerge(coordinateList A, coordinateList B) {
	...
	while (A.size() > 2 && B.size() > 2) // Check that we're not going to overflow
	{
	coordinate c, d, j, k;
	...
	while (findTurn(d,j,k) != 2 && findTurn(j,d,c) != 1 && A.size() > 2 && B.size() > 2)
	{
...
coordinateList convexHull::grahamScan(coordinateList G) {
	int GSize;
	GSize = (int) G.size();
...
coordinateList convexHull::doLowerHull (coordinateList lowerList) {
	...
	lowerOut.push_back(getCoord(lowerList,lowerList.size()-2));
	for (int b = lowerList.size() - 3; b >= 1; b--)
	{
		// push L[b] onto upperList;
		lowerOut.push_back(getCoord(lowerList,b));
		
		while (lowerOut.size() > 2 && findTurn(lowerOut[b-2],lowerOut[b-1],lowerOut[b]) == 2) {
	...
	printList(lowerOut);
	cout << "Lower hull distance: " << solveDistance(lowerOut) << endl;
	cout << "END LOWER HULL\n-----------------------------\n";
	return lowerOut;
} // end algorithm
...
coordinateList convexHull::doUpperHull(coordinateList upperList) {
	...
	for (int i = 2; i < sizeOfList; i++)
	{
		...
		while (upperOut.size() > 2 && findTurn(upperOut[i-2],upperOut[i-1],upperOut[i]) == 1) {
	...
	printList(upperOut);
	cout << "Lower hull distance: " << solveDistance(upperOut) << endl;
	cout << "END UPPER HULL\n-----------------------------\n";
	return upperOut;
}
